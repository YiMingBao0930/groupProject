...
interface UserNutritionLogDao {
    ...
    @Query("""
        SELECT 
            ...
            SUM(f.transFat * u.quantity) AS totalTransFat,
            SUM(f.polyUnsaturatedFat * u.quantity) AS totalPolySaturatedFat,
            SUM(f.monoUnsaturatedFat * u.quantity) AS totalMonoSaturatedFat,
